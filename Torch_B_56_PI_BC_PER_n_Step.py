...
class DDPGfDAgent:
    ...
    Attribute:
        ...
        PG_lambda1 (float): weight for policy gradient loss
        PG_lambda2 (float): weight for behavior cloning loss
    ...
    def __init__(
        ...
        # PG_loss parameters
        self.PG_lambda1 = PG_lambda1
        self.PG_lambda2 = PG_lambda2 / demo_batch_size
    ...
    def train_step(self) -> Tuple[torch.Tensor, ...]:
        ...
        # PG loss
        pg_loss_element_wise = -self.critic(state, self.actor(state))
        pg_loss = torch.mean(pg_loss_element_wise * weights)
        ...
        actor_loss = self.PG_lambda1 * pg_loss + self.PG_lambda2 * bc_loss
        ...
        new_priorities += self.lambda3 * pg_loss_element_wise.pow(2)
